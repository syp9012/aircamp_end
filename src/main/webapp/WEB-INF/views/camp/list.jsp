...
<body>
	...
	<section class="list-container">
		<p class="count">총 <span class="maincolor">${campCount}개의</span> 캠핑장이 검색되었습니다</p>
		<div class="list-wrap">
			<c:choose>
			    <c:when test="${campCount > 0}">
					<c:forEach var="list" items="${campList}">
						<div class="list">
							<a href="detail?camp_no=${list.camp_no}" class="inner"> 
								<c:choose>
		...
		<div class="pagination-wrap">
			<c:if test="${campCount > 0}">
				<!-- 첫페이지, 이전블록 으로 이동 -->
				<c:if test="${startPage > 1}">
					<a href="/camp/list?page=1&addr1=${search.addr1}&addr2=${search.addr2}&keyword=${search.keyword}" class="ico"> 
						<i class="fa-solid fa-chevron-left"></i><i class="fa-solid fa-chevron-left"></i>
					</a>
				</c:if>
				<c:if test="${startPage > 10}">
					<a href="/camp/list?page=${startPage - 10}&addr1=${search.addr1}&addr2=${search.addr2}&keyword=${search.keyword}" class="ico">
				...
				<c:forEach var="i" begin="${startPage}" end="${endPage}">
					<c:if test="${i == search.page}">
						<a href="/camp/list?page=${i}&addr1=${search.addr1}&addr2=${search.addr2}&keyword=${search.keyword}" class="on">${i}</a>
					</c:if>
					<c:if test="${i != search.page}">
				...
				<c:if test="${endPage < maxPage}">
					<a href="/camp/list?page=${maxPage}&addr1=${search.addr1}&addr2=${search.addr2}&keyword=${search.keyword}" class="ico">
